...
$(element).append('<div><form><textarea autofocus id="exec" spellcheck="false" autocapitalize="off" rows="1" autocorrect="off"></textarea></form>'
         + '<div id="console"><ul id="output"></ul></div></div>');

var execElement = $(element).find('#exec')[0];
var outputElement = $(element).find('#output')[0];
var formElement = $(element).find('form')[0];
...
if (enableCC) {
  ...
  exec = $(element).find('#exec')[0];
  cursor = $(element).find('#cursor')[0];
}
